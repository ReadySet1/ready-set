...
const Logo: React.FC<LogoProps> = ({ isHomePage, sticky, isVirtualAssistantPage, }) => {
    if (isVirtualAssistantPage) {
        return (<Link href="/" className={`navbar-logo block w-full ${sticky ? "py-3" : "py-6"}`}>
        {sticky ? (<picture>
...
const Header: React.FC = () => {
    ...
    useEffect(() => {
        uiLogger.debug("🆘 HEADER FALLBACK useEffect TRIGGERED - checking conditions:", {
            hasUser: !!user,
            isLoading,
            isSigningOut,
        ...
        if (isSigningOut) {
            uiLogger.debug("🆘 HEADER FALLBACK: Skipping due to sign-out in progress");
            return;
        }
        if (!user && !isLoading && typeof window !== "undefined") {
            uiLogger.debug("🆘🆘🆘 HEADER FALLBACK: No user from context, checking cookies manually...");
            ...
            if (sessionMatch && sessionMatch[1]) {
                try {
                    ...
                    uiLogger.debug("🆘 HEADER FALLBACK: Found session data:", sessionData);
                    ...
                    if (sessionData.userId && sessionData.email && sessionData.userRole) {
                        ...
                        if (hasAuthCookies()) {
                            ...
                            uiLogger.debug("✅ HEADER FALLBACK: Set fallback user successfully!");
                        }
                        else {
                            uiLogger.debug("🆘 HEADER FALLBACK: Session data found but no valid auth cookies, clearing fallback state");
                ...
                uiLogger.debug("🆘 HEADER FALLBACK: No session cookie found, clearing fallback state");
                setFallbackUser(null);
                setFallbackRole(null);
            }
        }
    ...
    const roleMenuItem = effectiveUserRole && typeof effectiveUserRole === "string"
    ...
    return (<header key={user?.id || "no-user"} className={`ud-header left-0 top-0 z-40 flex w-full items-center ${sticky
            ? "shadow-nav fixed z-[999] border-b border-stroke bg-white/80 backdrop-blur-[5px] dark:border-dark-3/20 dark:bg-dark/10"
            : "absolute bg-transparent"}`}>
      <div className="container">
        <div className="relative -mx-4 flex items-center justify-between">
          <div className="w-72 max-w-full px-4">
            <Logo isHomePage={isHomePage} sticky={sticky} isVirtualAssistantPage={isVirtualAssistantPage}/>
          </div>

          <div className="flex w-full items-center justify-between">
            <MobileMenu navbarOpen={navbarOpen} menuData={menuItems} openIndex={activeSubmenu} handleSubmenu={handleSubmenu} closeNavbarOnNavigate={closeNavbar} navbarToggleHandler={toggleNavbar} pathUrl={pathUrl} sticky={sticky} isHomePage={isHomePage} isVirtualAssistantPage={isVirtualAssistantPage} isLogisticsPage={isLogisticsPage}/>

            {/* Auth Buttons (only visible on desktop; mobile handled by MobileMenu) */}
            <div className="hidden items-center justify-end pr-16 sm:flex lg:pr-0">
              {isLoading ? (<AuthButtonsSkeleton sticky={sticky} isVirtualAssistantPage={isVirtualAssistantPage} isHomePage={isHomePage} isLogisticsPage={isLogisticsPage}/>) : !effectiveUser ? (<>
                  {pathUrl !== "/" || isVirtualAssistantPage ? (<div className="flex items-center gap-3">
                      <Link href="/sign-in" className={`hidden rounded-lg px-7 py-3 text-base font-semibold transition-all duration-300 lg:block ${sticky
                    ...
                    : isVirtualAssistantPage
                      ...
                      <Link href="/sign-in" className={`hidden rounded-lg px-7 py-3 text-base font-semibold transition-all duration-300 md:block ${sticky
                    ? "bg-white/90 text-dark shadow-md hover:bg-white dark:bg-gray-800 dark:text-white dark:hover:bg-gray-700"
                    : "bg-white/90 text-dark shadow-md hover:bg-white"} `}>
                        Sign In
                      </Link>
                      <Link href="/sign-up" className={`hidden rounded-lg px-6 py-3 text-base font-medium text-white duration-300 ease-in-out md:block ${sticky
                  ...
                  {roleMenuItem && roleMenuItem.path ? (<Link href={roleMenuItem.path} className={`loginBtn hidden px-7 py-3 font-medium lg:block ${sticky
                    ...
                    : isVirtualAssistantPage ||
                        ...
                        : "text-dark dark:text-white"}`}>
                      {roleMenuItem.title}
                    </Link>) : effectiveUserRole === null ? (<div className={`px-7 py-3 font-medium ${sticky
                    ? "text-dark dark:text-white"
                    : isVirtualAssistantPage ||
                        isHomePage ||
                        isLogisticsPage
                  ...
                  {isVirtualAssistantPage || isHomePage ? (<button onClick={handleSignOut} disabled={isSigningOut} className="signUpBtn hidden rounded-lg bg-blue-800 bg-opacity-20 px-6 py-3 text-base font-medium text-white duration-300 ease-in-out hover:bg-opacity-100 hover:text-white md:block">
                      {isSigningOut ? "Signing Out..." : "Sign Out"}
                    </button>) : (<button onClick={handleSignOut} disabled={isSigningOut} className="signUpBtn hidden rounded-lg bg-blue-800 bg-opacity-100 px-6 py-3 text-base font-medium text-white duration-300 ease-in-out hover:bg-opacity-20 hover:text-dark md:block">
                      {isSigningOut ? "Signing Out..." : "Sign Out"}
                    </button>)}
                </>) : null}
            </div>
            ...
            {/* Mobile Menu Toggle */}
            <button onClick={toggleNavbar} id="navbarToggler" aria-label="Mobile Menu" className="absolute right-4 top-1/2 block -translate-y-1/2 rounded-lg px-3 py-[6px] ring-primary focus:ring-2 lg:hidden">
              {[1, 2, 3].map((_, index) => (<span key={index} className={`relative my-1.5 block h-0.5 w-[30px] transition-all duration-300 ${navbarOpen
                ? index === 1
                    ? "opacity-0"
                    : index === 0
                        ? "top-[7px] rotate-45"
