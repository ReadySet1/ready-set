...
export function FileUploader({ onUpload, progresses = {}, isUploading = false, accept = {
    "image/*": [],
    "application/pdf": [],
}, maxSize = 5 * 1024 * 1024, // 5MB default
maxFileCount = 1, multiple = false, category, entityType, entityId, children, }: FileUploaderProps) {
    ...
    return (<div className="space-y-4">
      <div {...getRootProps()} className={cn("border-2 border-dashed rounded-md transition-colors", isTemporarilyDisabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer", isDragActive
            ? "border-primary/70 bg-primary/5"
            : "border-border hover:border-primary/50 hover:bg-muted/50", isUploading && "opacity-50 cursor-not-allowed")}>
        <input {...getInputProps()} disabled={isTemporarilyDisabled}/>
        {children || (<div className="flex flex-col items-center justify-center text-center space-y-2 p-6">
      ...
      {error && (<div className="text-sm text-red-500 flex items-center space-x-1">
          <X className="h-4 w-4"/>
          <span>{error}</span>
        </div>)}
      ...
      {selectedFiles.length > 0 && (<div className="space-y-2">
          ...
          <ul className="space-y-2">
            {selectedFiles.map((file, index) => (<li key={`${file.name}-${index}`} className="flex items-center justify-between text-sm p-2 border rounded-md">
          ...
          <Button size="sm" onClick={handleUpload} disabled={isUploading || selectedFiles.length === 0} className="mt-2">
            {isUploading ? (<>
      ...
      {isUploading && Object.keys(progresses).length > 0 && (<div className="space-y-2">
          {Object.entries(progresses).map(([filename, progress]) => (<div key={filename} className="space-y-1">
