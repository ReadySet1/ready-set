...
export class CarrierService {
     ...
     */
    static async sendStatusUpdate(orderNumber: string, driverStatus: DriverStatus, metadata?: Record<string, unknown>): Promise<WebhookResult | null> {
     ...
     */
    private static async executeWebhook(carrier: CarrierConfig, payload: WebhookPayload): Promise<WebhookResult> {
     ...
     */
    private static async makeWebhookRequest(carrier: CarrierConfig, payload: WebhookPayload): Promise<Record<string, unknown>> {
     ...
     */
    static async sendBulkStatusUpdates(updates: Array<{
        orderNumber: string;
        ...
        const results = await Promise.allSettled(updates.map(async ({ orderNumber, driverStatus, metadata }) => {
     ...
     */
    static async testConnections(): Promise<Record<string, {
        connected: boolean;
        latencyMs?: number;
        error?: string;
    }>> {
