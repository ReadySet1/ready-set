...
export async function login(prevState: FormState | null, formData: FormData): Promise<FormState> {
    ...
    const normalizedUserRole = userType ? Object.values(UserType).find(enumValue => enumValue.toUpperCase() === (userType as string)?.toUpperCase()) || (userType as string)?.toLowerCase() : 'customer';
    const sessionData = {
        userId: user.id,
        email: user.email || '',
