...
interface GoogleAuthButtonProps {
  className?: string;
  userType?: 'vendor' | 'client' | 'admin' | 'super_admin' | 'driver' | 'helpdesk';
  mode?: 'signup' | 'signin';
}
...
const GoogleAuthButton = ({ 
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const isDev = hostname === 'development.readysetllc.com' || 
                   hostname === 'localhost' || 
                   hostname.includes('local.readysetllc.com');
      
      // Construct the redirect URL
      let redirectUrl = isDev 
        ? `${window.location.protocol}//${hostname}/auth/callback`
        : getRedirectUrl();

      // Ensure the URL is properly formatted
      const redirectUrlObj = new URL(redirectUrl);
      
      // Add userType as a query parameter if provided
      if (userType) {
        redirectUrlObj.searchParams.set('userType', userType);
      }
      
      // Add mode as a query parameter
      redirectUrlObj.searchParams.set('mode', mode);
