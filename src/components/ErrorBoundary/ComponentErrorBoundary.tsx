...
import { createErrorBoundaryLogger, collectErrorContext, } from "@/lib/error-logging";
...
class ComponentErrorBoundary extends Component<Props, State> {
    ...
    private errorLogger: (error: Error, errorInfo: any, additionalContext?: Record<string, any>) => void;
    static defaultProps = {
        enableRetry: true,
        maxRetries: 2,
        showDetails: false,
    ...
    constructor(props: Props) {
        ...
        this.errorLogger = createErrorBoundaryLogger(props.componentName, "component");
    }
    ...
    handleRetry = (): void => {
        ...
        }, 500 + this.state.retryCount * 250); // Progressive delay
    };
    ...
    render(): ReactNode {
        if (this.state.hasError) {
            ...
            if (this.props.graceful) {
                return (<div className="rounded-md border border-orange-200 bg-orange-50 p-4 dark:bg-orange-900/20">
            ...
            {this.props.enableRetry &&
                        this.state.retryCount < (this.props.maxRetries || 2) && (<Button onClick={this.handleRetry} disabled={this.state.isRetrying} size="sm" variant="outline" className="mt-2 border-orange-300 text-orange-800 hover:bg-orange-100 dark:border-orange-600 dark:text-orange-200 dark:hover:bg-orange-900/30">
                  <RefreshCw className={`mr-1 h-3 w-3 ${this.state.isRetrying ? "animate-spin" : ""}`}/>
                  {this.state.isRetrying ? "Retrying..." : "Retry"}
                </Button>)}
          </div>);
            }
            // Full component replacement error UI
            return (<div className="rounded-lg border border-red-200 bg-red-50 p-6 dark:bg-red-900/20">
          <div className="flex items-start space-x-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {this.state.errorId && (<div className="mt-2 text-xs text-red-600 dark:text-red-400">
                  Error ID:{" "}
              ...
              {(this.props.showDetails ||
                    ...
                    this.state.error && (<details className="mt-3">
              ...
              <div className="mt-4 flex space-x-2">
                {this.props.enableRetry &&
                    this.state.retryCount < (this.props.maxRetries || 2) && (<Button onClick={this.handleRetry} disabled={this.state.isRetrying} size="sm" className="bg-red-600 text-white hover:bg-red-700">
                      <RefreshCw className={`mr-1 h-3 w-3 ${this.state.isRetrying ? "animate-spin" : ""}`}/>
                      {this.state.isRetrying ? "Retrying..." : "Retry"}
                    </Button>)}

                <Button onClick={() => window.location.reload()} size="sm" variant="outline" className="border-red-300 text-red-700 hover:bg-red-100 dark:border-red-600 dark:text-red-300 dark:hover:bg-red-900/30">
