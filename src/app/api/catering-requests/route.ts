...
export async function POST(request: NextRequest) {
    try {
        ...
        if (existingOrder) {
            return NextResponse.json({ message: "This order number already exists" }, { status: 409 });
        }
        ...
        if (!pickupAddr) {
            return NextResponse.json({ message: `Pickup address with ID ${data.pickupAddress.id} not found` }, { status: 400 });
        }
        const deliveryAddr = await prisma.address.findUnique({
            where: { id: data.deliveryAddress.id },
        });
        if (!deliveryAddr) {
            return NextResponse.json({ message: `Delivery address with ID ${data.deliveryAddress.id} not found` }, { status: 400 });
        }
        ...
        return NextResponse.json({ message: error instanceof Error ? error.message : "Failed to process catering request" }, { status: 500 });
    }
}
