...
const UserStatusCard: React.FC<UserStatusCardProps> = ({ user, onStatusChange, onRoleChange, currentUserRole, }) => {
      ...
      <CardContent>
        <div className="grid gap-6">
          <div className="grid gap-3">
            ...
            <Select onValueChange={handleStatusChange} value={currentStatus} disabled={!isSuperAdmin || isUpdating}>
              <SelectTrigger id="status" className="w-full">
                <SelectValue placeholder="Select status">
                  {isUpdating ? (<div className="flex items-center">
                      <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                      Updating...
                    </div>) : (currentStatus || "Select status")}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
            ...
            {!isSuperAdmin && (<p className="text-sm text-muted-foreground mt-1">
                ...
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" disabled={isUpdating} className="w-full justify-start">
                    {isUpdating ? (<div className="flex items-center">
                        <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                        Updating...
                      </div>) : (user.type?.charAt(0).toUpperCase() + user.type?.slice(1) || "Select Role")}
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent>
                  {availableRoles.map((role) => (<DropdownMenuItem key={role} onClick={() => handleRoleChange(role)} disabled={role === user.type}>
                      {role.charAt(0).toUpperCase() + role.slice(1)}
                    </DropdownMenuItem>))}
                </DropdownMenuContent>
              </DropdownMenu>) : (<div className="p-2 border rounded-md">
                {user.type?.charAt(0).toUpperCase() + user.type?.slice(1) || "Not set"}
              </div>)}
          </div>
        </div>
        
        {errorMessage && (<div className="mt-4 p-3 bg-red-50 border border-red-200 rounded text-red-600 text-sm">
            {errorMessage}
          </div>)}
      </CardContent>
      <CardFooter>
        <div className="w-full flex flex-col gap-2">
          ...
          <Button variant="outline" size="sm" onClick={testPermissions} disabled={isUpdating}>
            {isUpdating ? (<div className="flex items-center">
                <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                Testing...
