...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ error: 'Failed to generate file URL', details: error.message || String(error) }, { status: 500 });
    }
}
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (!rateLimitPassed) {
            const rateLimitError = UploadErrorHandler.createValidationError('content', 'Rate limit exceeded for file uploads', 'Too many upload attempts. Please wait a moment and try again.', { userId, action: 'UPLOAD' });
        ...
        let storageData: any, storageError: {
            message?: string;
        } | null = null;
        ...
        for (const strategy of uploadStrategies) {
            try {
                const result = await RetryHandler.withRetry(strategy, {
        ...
        try {
            ...
            fileUpload = await RetryHandler.withRetry(dbOperation, {
                ...DEFAULT_RETRY_CONFIG,
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        if (!fileUrl && !fileIdParam) {
            return NextResponse.json({ error: "Either fileUrl or fileId is required" }, { status: 400 });
        }
        ...
        return NextResponse.json({ error: "Failed to delete file", details: error.message || String(error) }, { status: 500 });
    }
}
