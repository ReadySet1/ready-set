...
export function OrderFilesManager({ orderNumber, order_type, orderId, initialFiles = [], }: OrderFilesManagerProps) {
    ...
    useEffect(() => {
        const fetchFiles = async () => {
            ...
            try {
                setIsLoading(true);
                const response = await fetch(`/api/orders/${encodeURIComponent(orderNumber)}/files`);
                if (!response.ok) {
                ...
                const processedFiles = filesArray.map((file: {
                    ...
                    return {
                        ...
                        userId: file.userId,
                        cateringRequestId: typeof file.cateringRequestId === "number"
                            ? file.cateringRequestId
                            : undefined,
                        onDemandId: typeof file.onDemandId === "number"
                            ? file.onDemandId
                            : undefined,
    ...
    return (<>
      ...
      <CardContent className="space-y-6">
        <FileUploader onUpload={handleUpload} progresses={progresses} isUploading={isUploading} category="catering-order" entityType="catering" entityId={orderId} accept={{
            "image/*": [],
            "application/pdf": [".pdf"],
        ...
        }} maxSize={5 * 1024 * 1024} maxFileCount={10} multiple={true}/>
        <UploadedFilesViewer files={allFiles} isLoading={isLoading} onFileClick={handleFileClick}/>
      </CardContent>
      <FileViewer file={selectedFile} isOpen={isViewerOpen} onClose={() => {
