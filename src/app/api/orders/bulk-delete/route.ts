...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!user || !user.id) {
            return NextResponse.json({ message: "Unauthorized - Must be signed in" }, { status: 401 });
        }
            ...
            try {
                ...
                await prisma.$transaction(async (tx: PrismaTransaction) => {
                    for (const file of fileUploads) {
                        ...
                        try {
                            if (!file.fileUrl) {
                                console.warn(`File record ${file.id} for order ${orderNumber} has no fileUrl.`);
                            ...
                            if (bucketNameIndex !== -1 &&
                                ...
                                if (simpleMatch && simpleMatch[1]) {
                                    ...
                                    console.warn(`Using fallback path extraction (likely just filename): ${filePath} for URL: ${file.fileUrl}. Verify this is correct!`);
                                }
                                else {
                                    console.error(`Could not extract valid file path for bucket ${BUCKET_NAME} from URL: ${file.fileUrl}`);
                                }
                            }
                        }
                        catch (e) {
                            console.error(`Error parsing fileUrl "${file.fileUrl}" for order ${orderNumber}:`, e);
                        ...
                        if (filePath) {
                            ...
                            try {
                                ...
                                if (storageError) {
                                    console.error(`Storage Error deleting ${filePath} (Order ${orderNumber}):`, storageError.message);
                                ...
                                console.error(`Exception during storage deletion for ${filePath} (Order ${orderNumber}):`, err);
                                // Again, decide if this should rollback the transaction
                                // throw err;
                            }
                        }
                        else {
                            console.warn(`Skipping storage deletion for file record ${file.id} (Order ${orderNumber}) due to missing/unparsable path from URL: ${file.fileUrl}`);
                        }
