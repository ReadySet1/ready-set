...
export function withRateLimit(config: RateLimitConfig | RateLimitTier) {
    return async function rateLimit(request: NextRequest): Promise<NextResponse | null> {
        ...
        message = 'Too many requests. Please try again later.', strategy = 'sliding-window', skipSuccessfulRequests = false, skipFailedRequests = false } = rateLimitConfig;
 ...
 */
export function createRateLimitedHandler<T extends any[], R>(handler: (request: NextRequest, ...args: T) => Promise<NextResponse>, rateLimitType: RateLimitTier = 'API') {
