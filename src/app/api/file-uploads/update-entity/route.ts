...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!oldEntityId || !newEntityId) {
            return NextResponse.json({ error: "Both old and new entity IDs are required" }, { status: 400 });
        }
        ...
        return NextResponse.json({ error: "Failed to update file entity IDs" }, { status: 500 });
    }
}
