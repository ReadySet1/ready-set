...
- `NSCameraUsageDescription` — the /driver POD photo uses `getUserMedia` in the
  WebView; without this key iOS denies the camera outright.

**Android — `android/app/src/main/AndroidManifest.xml`:**
- `ACCESS_FINE_LOCATION`, `ACCESS_COARSE_LOCATION`, `ACCESS_BACKGROUND_LOCATION`,
  `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_LOCATION`.
- `CAMERA` (+ optional `<uses-feature android:name="android.hardware.camera"
  android:required="false"/>`) — Capacitor only grants the WebView's
  `getUserMedia` camera request if the app itself declares CAMERA; without it the
  POD photo fails with "Camera permission was denied" (hit in the 2026-07-02
  Android walk test).
- The `@capacitor-community/background-geolocation` foreground-service entry (see
  the plugin README for the exact `<service>` block).

> ⚠️ `android/` and `ios/` are (re)generated by `cap add` — every permission
> above must be re-applied after regenerating a platform. The manifests are not
> under version control for Android (`android/` is gitignored), so this README
> is the checklist.
