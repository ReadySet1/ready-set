...
export default function UserDocumentsCard({ uploadHooks, userType, setRefreshTrigger, isUserProfile, }: UserDocumentsCardProps) {
    ...
    const handleDelete = async (fileId: string) => {
        try {
            const response = await fetch(`/api/file-uploads?fileId=${encodeURIComponent(fileId)}`, {
                method: "DELETE",
            });
            if (!response.ok) {
    ...
    return (<Card className="overflow-hidden">
      ...
      <CardContent className="p-6">
        ...
        {allUploadedFiles.length > 0 && (<div className="mb-4 space-y-2">
            <h4 className="text-sm font-medium text-slate-700">
              Uploaded Files
            </h4>
            <div className="divide-y divide-slate-200 rounded-md border border-slate-200">
              {allUploadedFiles.map((file) => (<div key={file.key} className="flex items-center justify-between p-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-slate-500">
                      ...
                      {file.category && (<span className="ml-2 text-xs text-slate-400">
                  ...
                  <div className="ml-4 flex space-x-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => handleDelete(file.key)}>
        ...
        <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4">
          ...
          <UserProfileUploads uploadHooks={uploadHooks} userType={userType} onUploadSuccess={() => setRefreshTrigger((prev) => prev + 1)}/>
        </div>
      </CardContent>
    </Card>);
}
