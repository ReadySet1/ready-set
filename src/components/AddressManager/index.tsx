...
const AddressManager: React.FC<AddressManagerProps> = ({ onAddressesLoaded, onAddressSelected, onError, defaultFilter = "all", showFilters = true, showManagementButtons = true, onRefresh, }) => {
    ...
    const fetchAddresses = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/addresses?filter=${filterType}&page=${currentPage}&limit=${limit}`, {
                headers: {
            ...
            setAddresses([]);
            if (onError) {
                onError(`Error fetching addresses: ${err instanceof Error ? err.message : "Unknown error"}`);
            }
        }
        finally {
            setIsLoading(false);
    ...
    const handleAddAddress = useCallback(async (newAddress: Partial<Address>) => {
        const { data: { session }, error: sessionError, } = await supabase.auth.getSession();
        if (sessionError || !session) {
    ...
    return (<div className="address-manager w-full space-y-4">
      {error && (<div className="rounded-md border border-red-200 bg-red-50 p-4 text-red-700">
          <div className="flex items-start">
            ...
            <div className="ml-auto pl-3">
              <Button variant="ghost" size="sm" onClick={() => setError(null)} className="text-red-700 hover:bg-red-100 hover:text-red-900">
                Dismiss
              </Button>
            </div>
          </div>
      ...
      {showFilters && (<Tabs defaultValue={filterType} onValueChange={handleFilterChange} className="w-full">
      ...
      {isLoading ? (<div className="flex items-center justify-center p-4">
          ...
          <Controller name="pickUpLocation" control={control} render={({ field }) => (<Select value={selectedAddressId} onValueChange={handleAddressSelection}>
                ...
                <SelectContent>
                  <SelectGroup>
                    ...
                    addresses
                        ...
                        .map((address) => (<SelectItem key={address.id} value={address.id}>
                  ...
                  {Array.isArray(addresses) &&
                    addresses.some((a) => a.isShared) && (<SelectGroup>
      ...
      {showManagementButtons && (<div className="flex flex-col gap-3 pb-6 sm:flex-row sm:space-x-4">
          <Link href="/addresses" className="rounded-md bg-blue-500 px-4 py-2 text-center text-white transition hover:bg-blue-600 sm:text-left">
            Manage Addresses
          </Link>

          <Button onClick={handleToggleAddForm} variant={showAddForm ? "destructive" : "default"} className={showAddForm
      ...
      {showAddForm && (<div ref={addressFormRef}>
          <AddAddressForm onSubmit={async (data) => {
                try {
            ...
            }} onClose={() => setShowAddForm(false)} initialValues={{
                isShared: false,
                isRestaurant: false,
            }}/>
        </div>)}
      ...
      {/* Pagination Section */}
      {!isLoading && pagination.totalPages > 1 && (<div className="mt-6 flex items-center justify-between border-t bg-slate-50 p-4">
          <Pagination>
            <PaginationContent className="flex-wrap gap-1">
              <PaginationItem>
                <PaginationPrevious onClick={handlePrevPage} className={`${!pagination.hasPrevPage ? "pointer-events-none opacity-50" : "cursor-pointer hover:bg-slate-200"} text-sm`}/>
              </PaginationItem>
              {/* Basic Pagination - Consider a more advanced version for many pages */}
              {[...Array(pagination.totalPages)].map((_, i) => (<PaginationItem key={i} className="hidden sm:block">
                  <PaginationLink onClick={() => handlePageChange(i + 1)} isActive={pagination.currentPage === i + 1} className={`cursor-pointer text-sm ${pagination.currentPage === i + 1 ? "bg-amber-100 text-amber-800 hover:bg-amber-200" : "hover:bg-slate-200"}`}>
                    {i + 1}
                  </PaginationLink>
                </PaginationItem>))}
              ...
              <PaginationItem>
                <PaginationNext onClick={handleNextPage} className={`${!pagination.hasNextPage ? "pointer-events-none opacity-50" : "cursor-pointer hover:bg-slate-200"} text-sm`}/>
              </PaginationItem>
            </PaginationContent>
          </Pagination>
        </div>)}
