...
import { NavigationIcon, ClockIcon, MapPinIcon, TruckIcon, BatteryIcon, SignalIcon, PlayIcon, PauseIcon, SquareIcon, CoffeeIcon, AlertTriangleIcon, CheckCircleIcon } from 'lucide-react';
...
export default function DriverTrackingPortal({ className }: DriverTrackingPortalProps) {
    ...
    const { currentLocation, isTracking, accuracy, startTracking, stopTracking, error: locationError } = useLocationTracking();
    const { currentShift, isShiftActive, startShift, endShift, startBreak, endBreak, loading: shiftLoading, error: shiftError } = useDriverShift();
    const { activeDeliveries, updateDeliveryStatus, loading: deliveriesLoading, error: deliveriesError } = useDriverDeliveries();
    ...
    const handleEndShift = async () => {
        if (!currentLocation || !currentShift?.id)
            return;
    ...
    return (<div className={cn('w-full max-w-md mx-auto p-4 space-y-4', className)}>
      {/* Header Status Bar */}
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            ...
            'text-red-500': !accuracy || accuracy >= 100
              ...
              {/* Battery Level */}
              {batteryLevel !== null && (<div className="flex items-center space-x-1">
      ...
      {/* Error Alerts */}
      {(locationError || shiftError || deliveriesError) && (<Alert variant="destructive">
        ...
        <CardContent className="space-y-4">
          {isShiftActive && currentShift ? (<>
              ...
              <div className="space-y-2">
                {isOnBreak && currentBreak ? (<div className="space-y-2">
                    <p className="text-sm text-center text-muted-foreground">
                      On {currentBreak.breakType} break since{' '}
                      {new Date(currentBreak.startTime).toLocaleTimeString()}
                    </p>
                    <Button onClick={handleEndBreak} className="w-full" variant="outline">
                      <PlayIcon className="w-4 h-4 mr-2"/>
                      End Break
                    </Button>
                  </div>) : (<div className="grid grid-cols-2 gap-2">
                    <Button onClick={() => handleStartBreak('rest')} variant="outline" size="sm">
                      <CoffeeIcon className="w-4 h-4 mr-1"/>
                      Break
                    </Button>
                    <Button onClick={() => handleStartBreak('meal')} variant="outline" size="sm">
              ...
              <Button onClick={handleEndShift} className="w-full" variant="destructive" disabled={shiftLoading}>
              ...
              <Button onClick={handleStartShift} className="w-full" disabled={shiftLoading || !currentLocation}>
                <PlayIcon className="w-4 h-4 mr-2"/>
                Start Shift
              </Button>

              {!currentLocation && (<p className="text-xs text-center text-muted-foreground">
      ...
      {/* Active Deliveries */}
      {activeDeliveries.length > 0 && (<Card>
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
          ...
          <CardContent className="space-y-3">
            {activeDeliveries.map((delivery) => (<div key={delivery.id} className="border rounded-lg p-3 space-y-2">
                <div className="flex items-center justify-between">
                ...
                <div className="text-sm text-muted-foreground">
                  ...
                  {delivery.estimatedArrival && (<p>⏰ ETA: {new Date(delivery.estimatedArrival).toLocaleTimeString()}</p>)}
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <Button size="sm" variant="outline" onClick={() => updateDeliveryStatus(delivery.id, DriverStatus.EN_ROUTE_TO_CLIENT, currentLocation || undefined)}>
                    En Route
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => updateDeliveryStatus(delivery.id, DriverStatus.ARRIVED_TO_CLIENT, currentLocation || undefined)}>
                    Arrived
                  </Button>
                </div>
                
                <Button size="sm" className="w-full" onClick={() => updateDeliveryStatus(delivery.id, DriverStatus.COMPLETED, currentLocation || undefined)}>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            ...
            {isTracking && (<span className="text-xs text-muted-foreground">
                Updates every 30s
              </span>)}
          </div>
        </CardContent>
