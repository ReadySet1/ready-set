...
export default function UserProfile({ userId: propUserId, isUserProfile = true }: UserProfileProps = {}) {
    ...
    const { loading, fetchUser, handleUploadSuccess, useUploadFileHook, userData } = useUserData(userId || "", refreshTrigger, setRefreshTrigger);
    // Destructure methods object and custom properties from the updated hook
    const { methods, // The full methods object from useForm
    watchedValues, hasUnsavedChanges, onSubmit } = useUserForm(userId || "", userData, fetchUser);
    ...
    if (isUserLoading && !session) {
        return (<div className="flex h-full min-h-[50vh] items-center justify-center">
    ...
    return (<div className="w-full">
      ...
      <FormProvider {...methods}> 
        <div className="w-full">
          ...
          <UserProfileHeader handleDiscard={handleDiscard} handleSubmit={handleSubmit} onSubmit={onSubmit} hasUnsavedChanges={hasUnsavedChanges} loading={loading}/>
          ...
          <div className="px-4 sm:px-6 lg:px-8 pt-6 pb-8"> 
            ...
            <div className="mt-8 grid gap-8 lg:grid-cols-3">
              ...
              <div className="col-span-2 space-y-8">
                <UserProfileTabs userId={userId} activeTab={activeTab} setActiveTab={memoizedSetActivetab} watchedValues={watchedValues} control={control} refreshTrigger={refreshTrigger} isUserProfile={true} // New prop to disable admin-specific features
    />
              </div>

              {/* Right Column - Files & Documents only, no status management */}
              <div>
                <UserDocumentsCard uploadHooks={uploadHooks} userType={watchedValues.type ?? "client"} setRefreshTrigger={setRefreshTrigger} isUserProfile={true}/>
              </div>
            </div>
...
const UserProfileHeader = ({ handleDiscard, handleSubmit, onSubmit, hasUnsavedChanges, loading, }: UserProfileHeaderProps) => (<div className="sticky top-0 z-40 border-b bg-white dark:bg-gray-800 shadow-sm">
    <div className="mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex h-16 items-center justify-between gap-4">
        <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">My Profile</h1>
        <div className="flex items-center gap-3">
          <Button variant="outline" size="default" onClick={handleDiscard} disabled={!hasUnsavedChanges || loading} className="border-gray-200 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-700">
            <XCircle className="mr-2 h-4 w-4"/>
            Discard
          </Button>
          <Button size="default" onClick={handleSubmit(onSubmit)} disabled={!hasUnsavedChanges || loading} className="bg-green-600 hover:bg-green-700 text-white dark:bg-green-700 dark:hover:bg-green-800">
