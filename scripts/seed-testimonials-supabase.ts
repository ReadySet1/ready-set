...
/**
 * Script to seed testimonials into the database using Supabase MCP
 * This script inserts all testimonials from the page.ts file into the testimonials table
 */
...
// Create a direct Supabase client for standalone scripts
...
// Testimonials data from src/components/Testimonials/page.ts
const testimonialsData = [
  ...
  },
  {
    ...
    text: 'As a frequent presenter with CareerLearning, it was always a pleasure to work with Kaleb. From the start, he impressed me with his professionalism and dedication to delivering a high-quality, collaborative experience for trainers. His promotions within CareerLearning were well deserved, and I am sure his initiative and proactive approach were part of his success. Kaleb is a valuable asset to any team, and I highly recommend him for any role requiring strong brand management, organization, and communication skills.',
    image: '/images/testimonials/author-09.png'
  },
  {
    category: 'CLIENTS',
    name: 'Crystal Rapada',
    role: 'VP of Business Development and Sales',
  ...
  {
    category: 'CLIENTS',
    name: 'Mary Gladstone-Highland',
    role: 'Certified Nonprofit Professional',
    text: "I had the pleasure of working with Bon as a presenter for Career Learning, where she served as the Program Coordinator and Curriculum Resource. Bon was an outstanding team member whose dedication and organizational skills were critical to the success of every session. Bon ensured everything ran smoothly, from scheduling presentations to preparing materials and managing customer questions. Her attention to detail and proactive communication made my role as a presenter much more accessible and helped create a seamless experience for participants. Bon also excelled in customer engagement. She was always prompt, thoughtful, and professional in responding to inquiries, ensuring that participants felt supported and valued. Her ability to manage multiple tasks while maintaining such a high standard of service was impressive and contributed significantly to Learning's reputation for excellence. I wholeheartedly recommend Bon for any role where strong organizational skills, effective communication, and a customer-first approach are She'sd. She is a true professional and a tremendous asset to any team.",
...
async function seedTestimonials() {
  ...
  try {
    ...
    if (deleteError) {
      console.error('❌ Error clearing existing testimonials:', deleteError);
      throw deleteError;
    }
    ...
    for (let i = 0; i < testimonialsData.length; i++) {
      ...
      if (error) {
        console.error(`❌ Error inserting testimonial ${i + 1} (${testimonial.name}):`, error);
        throw error;
      }

      console.log(`✅ Testimonial ${i + 1}/${testimonialsData.length}: ${testimonial.name} (${testimonial.category})`);
    }

    console.log('🎉 All testimonials seeded successfully!');

    // Verify the seeding
    console.log('🔍 Verifying testimonials in database...');
    ...
    console.log(`📊 Verification: Found ${testimonials?.length || 0} testimonials in database`);
    ...
    Object.entries(grouped).forEach(([category, items]) => {
      console.log(`  ${category}: ${items.length} testimonials`);
    });

    console.log('✅ Testimonials seeding completed successfully!');

  } catch (error) {
    console.error('❌ Error during testimonials seeding:', error);
    process.exit(1);
  }
}
