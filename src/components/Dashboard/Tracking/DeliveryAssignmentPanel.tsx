...
import { TruckIcon, MapPinIcon, ClockIcon, UserIcon, PlusIcon, SearchIcon, AlertTriangleIcon, CheckCircleIcon, XCircleIcon, CalendarIcon } from 'lucide-react';
...
export default function DeliveryAssignmentPanel({ drivers, deliveries, className }: DeliveryAssignmentPanelProps) {
    ...
    const availableDrivers = drivers.filter(driver => driver.isOnDuty && (driver.activeDeliveries || 0) < 3 // Max 3 deliveries per driver
    );
    ...
    const DeliveryCard = ({ delivery }: {
        ...
        return (<Card className={cn('cursor-pointer transition-all duration-200 hover:shadow-md', isSelected && 'ring-2 ring-blue-500', {
        ...
        <CardContent className="p-4">
          <div className="flex items-start justify-between">
            ...
            <div className="flex-1">
              ...
              {/* Timing */}
              {delivery.estimatedArrival && (<div className="flex items-center space-x-2 text-sm mt-2">
            ...
            <div className="flex flex-col items-end space-y-2">
              {assignedDriver ? (<div className="text-right">
                ...
                </div>) : (<div className="flex items-center space-x-1 text-sm text-red-600">
          ...
          {/* Assignment panel */}
          {isSelected && (<div className="mt-4 pt-4 border-t">
              {assignedDriver ? (<div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between p-3 bg-green-100 rounded-lg">
                    ...
                    <Button variant="outline" size="sm" onClick={() => handleAssignDelivery(delivery.id, '')} disabled={assignmentLoading}>
                      Unassign
                    </Button>
                  </div>
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {availableDrivers.length === 0 ? (<div className="text-center py-4 text-muted-foreground">
                        <XCircleIcon className="w-8 h-8 mx-auto mb-2"/>
                        <p>No available drivers</p>
                      </div>) : (availableDrivers.map(driver => (<div key={driver.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50 cursor-pointer" onClick={() => handleAssignDelivery(delivery.id, driver.id)}>
                          <div className="flex items-center space-x-3">
    ...
    return (<div className={cn('w-full space-y-4', className)}>
      ...
      <div className="flex items-center space-x-4">
        <div className="flex-1 relative">
          ...
          <Input placeholder="Search deliveries..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10"/>
        </div>
        
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value as typeof filterStatus)} className="px-3 py-2 border rounded-md text-sm">
      ...
      <div className="space-y-3">
        {filteredDeliveries.length === 0 ? (<div className="text-center py-8 text-muted-foreground">
            <TruckIcon className="w-12 h-12 mx-auto mb-2"/>
            <p>No deliveries match your criteria</p>
          </div>) : (filteredDeliveries.map(delivery => (<DeliveryCard key={delivery.id} delivery={delivery}/>)))}
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {availableDrivers.map(driver => (<div key={driver.id} className="flex items-center space-x-3 p-2 border rounded">
