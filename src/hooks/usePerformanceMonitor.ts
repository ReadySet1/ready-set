...
export function usePerformanceMonitor() {
    ...
    const detectSuspiciousPatterns = useCallback((newCall: ApiCallLog) => {
        const recentCalls = callLogRef.current.filter(call => newCall.timestamp - call.timestamp < 10000 // Last 10 seconds
        );
        if (recentCalls.length >= 3) {
            ...
            const suspiciousIntervals = intervals.filter(interval => interval >= 400 && interval <= 600);
    ...
    const checkForInfiniteLoops = useCallback(() => {
        const recentCalls = callLogRef.current.filter(call => Date.now() - call.timestamp < 5000 // Last 5 seconds
        );
    ...
    const monitorFetch = useCallback(async (url: string, options: RequestInit = {}): Promise<Response> => {
    ...
    const getPerformanceReport = useCallback(() => {
        ...
        const recentCalls = history.filter(call => Date.now() - call.timestamp < 60000 // Last minute
        );
...
export function useApiEndpointMonitor(endpoint: string) {
    ...
    const monitorEndpoint = useCallback(async (url: string, options: RequestInit = {}): Promise<Response> => {
        const startTime = Date.now();
        const method = options.method || 'GET';
        try {
            const response = await fetch(url, options);
