...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
            return NextResponse.json({ success: false, error: 'Unauthorized - Invalid authorization header' }, { status: 401 });
        }
        // Extract the token
        ...
        const history = await CalculatorService.getCalculationHistory(supabase, {
