 ...
 */
export async function updateCaterValleyOrderStatus(orderNumber: string, status: CaterValleyOrderStatus): Promise<CaterValleyUpdateResult> {
    const CATER_VALLEY_API_URL = process.env.NEXT_PUBLIC_CATER_VALLEY_API_URL || 'https://api.catervalley.com/api/operation/order/update-order-status';
...
export async function updateCaterValleyOrderStatusLegacy(orderNumber: string, status: CaterValleyOrderStatus): Promise<CaterValleyApiResponse> {
