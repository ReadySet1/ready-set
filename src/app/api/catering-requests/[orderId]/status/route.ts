...
export async function PATCH(request: NextRequest, context: {
    ...
    try {
        ...
        try {
            const webhookResult = await CarrierService.sendStatusUpdate(order.orderNumber, validatedData.driverStatus);
            if (webhookResult) {
        ...
        if (error instanceof Error) {
            if (error.message.includes('Record to update not found')) {
                return NextResponse.json({ error: 'Order not found or has been deleted' }, { status: 404 });
            }
        }
        return NextResponse.json({ error: 'Internal server error - failed to update order status' }, { status: 500 });
    }
}
/**
...
export async function GET(request: NextRequest, context: {
    ...
    try {
        ...
        return NextResponse.json({ error: 'Internal server error - failed to fetch order status' }, { status: 500 });
    }
}
