...
export async function PUT(request: NextRequest) {
    try {
        ...
        try {
            ...
            const isAdminOrHelpdesk = requesterProfile?.type === UserType.ADMIN ||
                requesterProfile?.type === UserType.SUPER_ADMIN ||
                requesterProfile?.type === UserType.HELPDESK;
            // Only allow if requesting own profile or admin/super_admin
            const isSelf = user.id === userId;
        ...
        if (requestBody.type) {
            try {
                ...
                if (Object.keys(UserType).includes(typeKey)) {
                    ...
                    return NextResponse.json({ error: `Invalid user type: ${requestBody.type}. Valid types are: ${Object.keys(UserType).map(k => k.toLowerCase()).join(', ')}` }, { status: 400 });
                }
            }
            catch (error) {
                console.error('[PUT /api/users/[userId]] Error converting user type:', error);
                return NextResponse.json({ error: `Invalid user type: ${requestBody.type}` }, { status: 400 });
            }
        }
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        if (requesterProfile?.type !== UserType.SUPER_ADMIN && requesterProfile?.type !== UserType.ADMIN) {
            return NextResponse.json({ error: 'Forbidden: Only Admin or Super Admin can delete users' }, { status: 403 });
        }
        ...
        if (userToDelete.type === UserType.SUPER_ADMIN) {
            return NextResponse.json({ error: 'Forbidden: Super Admin users cannot be deleted' }, { status: 403 });
        }
        ...
        const transactionResult = await prisma.$transaction(async (tx: PrismaTransaction) => {
            ...
            for (const address of createdAddresses) {
                const isUsedByOthers = address.userAddresses.some((ua: {
                    userId: string;
                }) => ua.userId !== userId) ||
