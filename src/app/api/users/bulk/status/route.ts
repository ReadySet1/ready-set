...
import type { BulkStatusChangeRequest } from "@/types/bulk-operations";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const bulkRequest: BulkStatusChangeRequest = {
      userIds,
      status,
      reason,
    };

    const results = await userBulkOperationsService.bulkStatusChange(
      bulkRequest,
      user.id
    );

    return NextResponse.json({
      message: `Bulk status change completed. ${results.totalSuccess} succeeded, ${results.totalFailed} failed.`,
      results,
    });
  } catch (error) {
