...
type DeliveryFormData = BakeryFormData | FlowerFormData | FoodFormData | SpecialtyFormData;
...
const formatSpecificFields = (data: DeliveryFormData): string => {
    switch (data.formType) {
        ...
        <ul>
          <li><strong>Delivery Types:</strong> ${data.deliveryTypes.map(type => type === 'bakedGoods' ? 'Baked Goods to Your Client' :
                'Supplies to Your Store').join(', ')}</li>
      ...
      `;
        ...
        <ul>
          <li><strong>Delivery Types:</strong> ${data.deliveryTypes.map(type => type === 'floralArrangements' ? 'Floral Arrangements' :
                'Floral Supplies').join(', ')}</li>
      ...
      `;
        ...
        <ul>
          ...
          <li><strong>Delivery Times:</strong> ${data.deliveryTimes.map(time => time.charAt(0).toUpperCase() + time.slice(1)).join(', ')}</li>
      ...
      `;
        ...
        <ul>
          <li><strong>Delivery Types:</strong> ${data.deliveryTypes.map(type => type === 'specialDelivery' ? 'Special Delivery' :
                'Specialty Delivery').join(', ')}</li>
