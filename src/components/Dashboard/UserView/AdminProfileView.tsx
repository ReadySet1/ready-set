...
export default function ModernUserProfile({ userId, isUserProfile = false, }: ModernUserProfileProps) {
    ...
    const { loading, isUpdatingStatus, fetchUser, handleStatusChange: baseHandleStatusChange, handleRoleChange, handleUploadSuccess, useUploadFileHook, } = useUserData(userId, refreshTrigger, setRefreshTrigger);
    // Initialize form hook, passing only userId and fetchUser
    const formMethods = useUserForm(userId, fetchUser);
    const { control, handleSubmit, watchedValues, hasUnsavedChanges, onSubmit, reset, setValue, formState: { isDirty }, } = formMethods;
    ...
    return (<FormProvider {...formMethods}>
      <div className="bg-muted/20 min-h-screen pb-10">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          {/* Header Navigation */}
          <HeaderNavigation handleBack={handleBack} handleDiscard={handleDiscard} handleSubmit={handleSubmit} onSubmit={onSubmit} hasUnsavedChanges={hasUnsavedChanges} loading={loading || isUpdatingStatus} isUserProfile={isUserProfile}/>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-3">
            ...
            <div className="col-span-2 space-y-6">
              <UserProfileTabs userId={userId} activeTab={activeTab} setActiveTab={memoizedSetActivetab} watchedValues={watchedValues} control={control} refreshTrigger={refreshTrigger} isUserProfile={isUserProfile}/>
            </div>

            {/* Right Column: Sidebar Cards */}
            <div className="space-y-6">
              <UserStatusCard watchedValues={watchedValues} control={control} isUpdatingStatus={isUpdatingStatus} loading={loading} handleStatusChange={handleStatusChange} handleRoleChange={handleRoleChange}/>

              <UserDocumentsCard uploadHooks={uploadHooks} userType={watchedValues.type ?? "client"} setRefreshTrigger={setRefreshTrigger}/>
...
const AuthenticationRequired = ({ router }: {
    router: any;
...
const ProfileSkeleton = () => (<div className="p-8">
...
const HeaderNavigation = ({ handleBack, handleDiscard, handleSubmit, onSubmit, hasUnsavedChanges, loading, isUserProfile = false, }: HeaderNavigationProps) => (<div className="sticky top-0 z-10 mb-6 border-b bg-white py-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={handleBack} className="text-muted-foreground hover:text-foreground h-9 w-9 dark:text-gray-400 dark:hover:text-gray-200">
            <ChevronLeft className="h-5 w-5"/>
          </Button>
          {!isUserProfile && (<div className="text-muted-foreground text-sm dark:text-gray-400">
              <Link href="/admin" className="hover:text-foreground hover:underline dark:hover:text-gray-200">
                Dashboard
              </Link>
              {" / "}
              <Link href="/admin/users" className="hover:text-foreground hover:underline dark:hover:text-gray-200">
                Users
              </Link>
              {" / "}
          ...
          {isUserProfile && (<h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">
              My Profile
            </h1>)}
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleDiscard} disabled={!hasUnsavedChanges || loading} className="h-9 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">
            <XCircle className="mr-1.5 h-4 w-4"/>
            Discard
          </Button>
          <Button size="sm" onClick={handleSubmit(onSubmit)} disabled={!hasUnsavedChanges || loading} className="h-9 bg-gradient-to-r from-blue-500 to-indigo-600 text-white hover:from-blue-600 hover:to-indigo-700 dark:from-blue-600 dark:to-indigo-700 dark:hover:from-blue-700 dark:hover:to-indigo-800">
