...
export class CaterValleyWebhookService {
     ...
     */
    static async sendStatusUpdate(orderNumber: string, driverStatus: DriverStatus, options: {
        maxRetries?: number;
        timeoutMs?: number;
    ...
    }>): Promise<Array<WebhookDeliveryResult & {
        orderNumber: string;
    }>> {
        const results = await Promise.allSettled(updates.map(async ({ orderNumber, driverStatus }) => {
