 ...
 */
export function withPerformanceMonitoring<T extends any[], R>(name: string, fn: (...args: T) => Promise<R>): (...args: T) => Promise<R> {
 ...
 */
export async function measureQuery<T>(queryName: string, query: () => Promise<T>, metadata?: Record<string, any>): Promise<T> {
    return performanceMonitor.measure(`DB: ${queryName}`, query, metadata);
}
/**
 * Utility for measuring API call performance
 */
export async function measureApiCall<T>(endpoint: string, apiCall: () => Promise<T>, metadata?: Record<string, any>): Promise<T> {
