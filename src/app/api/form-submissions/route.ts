...
export async function POST(req: Request) {
    try {
        ...
        if (!formType || !validFormTypes.includes(formType)) {
            return NextResponse.json({ error: `Invalid form type. Valid types are: ${validFormTypes.join(', ')}` }, { status: 400 });
        }
        ...
        if (!body.formData?.counties?.length) {
            return NextResponse.json({ error: 'At least one county must be selected' }, { status: 400 });
        }
