    ...
    try {
        ...
        if (!email || !password || !phoneNumber || !userType) {
            return NextResponse.json({
                ...
                missingFields: ["email", "password", "phoneNumber", "userType"].filter((field) => !body[field as keyof RequestBody]),
            }, { status: 400 });
        }
        ...
        if (existInPrisma) {
            return NextResponse.json({ error: "User already exists in our system!" }, { status: 400 });
        }
        let authData: {
            user: any;
        } | null = null;
        ...
        if (authError) {
            ...
            if (authError.message?.includes('User already registered') || authError.status === 422) {
                return NextResponse.json({ error: "Account found", details: "An account with this email already exists. Please try signing in instead." }, { status: 400 });
            }
            return NextResponse.json({ error: "Failed to create user in authentication system", details: authError.message }, { status: 500 });
        }
        if (!authData?.user) {
            return NextResponse.json({ error: "Failed to create user account", details: "No user data returned" }, { status: 500 });
        }
        ...
        const address = await prisma.address.create({
            data: {
                ...
                state: body.state,
                zip: body.zip,
                county: userType === UserType.VENDOR || userType === UserType.CLIENT
                ...
                parkingLoading: "parking" in body
