...
import { MoreVertical, Truck, Edit, Trash2, Calendar, Download, Printer, RefreshCw, ArrowUpDown, Loader2 } from "lucide-react";
...
const OrderHeader: React.FC<OrderHeaderProps> = ({ orderNumber, date, driverInfo, onAssignDriver, order_type, orderId, onDeleteSuccess, }) => {
    ...
    const handleDeleteOrder = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            const deletePromise = fetch(`/api/orders/delete?orderId=${orderId.toString()}&orderType=${apiOrderType}`, {
                method: "DELETE",
            });
    ...
    return (<>
      <CardHeader className="flex flex-row items-center justify-between p-6 border-b bg-slate-50">
        ...
        <div className="flex items-center gap-2">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button size="icon" variant="outline" onClick={handleRefreshOrder} disabled={isRefreshing} className="text-slate-600 hover:text-amber-600 transition-colors">
          ...
          <Button size="sm" variant="outline" className="gap-1.5 text-slate-600 hover:text-amber-600 border-slate-200 hover:border-amber-200 transition-colors hidden md:flex" onClick={onAssignDriver}>
            {driverInfo ? (<>
          ...
          <Button size="icon" variant="outline" className="md:hidden" onClick={onAssignDriver}>
            {driverInfo ? <Edit className="h-4 w-4"/> : <Truck className="h-4 w-4"/>}
          </Button>
          
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button size="icon" variant="outline" className="text-slate-600 hover:text-amber-600 border-slate-200 hover:border-amber-200 transition-colors">
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => setIsDeleteDialogOpen(true)} className={`text-red-600 ${isDeleting
            ? 'opacity-60 cursor-not-allowed'
            : 'hover:text-red-700 hover:bg-red-50 cursor-pointer'}`} disabled={isDeleting}>
                {isDeleting ? (<>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={(open) => {
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel className="border-slate-200" disabled={isDeleting}>
              Cancel
            </AlertDialogCancel>
            <Button onClick={(e) => handleDeleteOrder(e)} className="bg-red-600 hover:bg-red-700 text-white" disabled={isDeleting}>
              {isDeleting ? (<>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin"/>
                  Deleting...
                </>) : ('Delete Order')}
