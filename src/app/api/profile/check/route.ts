...
export async function GET(request: Request) {
    try {
        ...
        if (!isOAuth) {
            ...
            const shouldBypass = bypassProfiles.split(',').includes(user.email || '') ||
                bypassDomains.split(',').some(domain => user.email?.endsWith(domain) && domain !== '');
            if (shouldBypass) {
...
async function createUserProfile(supabase: SupabaseClient, userId: string, user: any): Promise<boolean> {
    try {
