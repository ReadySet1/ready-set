...
const sendRegistrationEmail = async (email: string, temporaryPassword: string) => {
    const body = `
...
export async function POST(request: Request) {
    try {
        ...
        if (supabaseError) {
            ...
            return NextResponse.json({ error: "Failed to create user account", details: supabaseError.message }, { status: 500 });
        }
        if (!authData.user) {
        ...
        if (newUser.email && body.generateTemporaryPassword) {
            ...
            await sendRegistrationEmail(newUser.email, password as string);
        }
