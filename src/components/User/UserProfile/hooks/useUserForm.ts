...
export const useUserForm = (userId: string, initialData: UserFormValues | null, refetchData: () => Promise<any>): UseUserFormReturn => {
    ...
    const onSubmit = async (data: UserFormValues) => {
        try {
            ...
            const { displayName, countiesServed, timeNeeded, cateringBrokerage, provisions, type, ...baseSubmitData } = data;
            ...
            toast.error(`Failed to save user: ${error instanceof Error ? error.message : "Please try again."}`);
        }
    };
    return {
        methods,
