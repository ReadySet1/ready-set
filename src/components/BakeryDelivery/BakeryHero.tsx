...
const BakeryHero: React.FC<BakeryHeroProps> = ({ onRequestQuote }) => {
    ...
    return (<section className={`relative w-full ${marginTopClass} overflow-hidden`}>
      {/* Background image */}
      <div className="absolute inset-0 z-0">
        <Image src="/images/food/truckbg.png" alt="Delivery truck background" fill sizes="100vw" priority style={{ objectFit: "cover" }}/>
      </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="relative z-10 w-full max-w-xl space-y-6 px-4 sm:px-6 lg:w-1/2 lg:px-0">
            ...
            <div className="flex flex-col items-center gap-4 pt-4 sm:flex-row">
              <button onClick={handleQuoteClick} className="w-full rounded-full bg-yellow-300 px-8 py-3 font-[Montserrat] font-bold text-gray-800 transition-colors hover:bg-yellow-400 sm:w-auto">
                Get a Quote
              </button>
              <ScheduleDialog buttonText="Book a Call" calendarUrl="https://calendar.google.com/calendar/appointments/schedules/AcZssZ0J6woLwahSRd6c1KrJ_X1cOl99VPr6x-Rp240gi87kaD28RsU1rOuiLVyLQKleUqoVJQqDEPVu?gv=true" className="mt-3 w-full rounded-full bg-yellow-300 px-8 py-3 font-[Montserrat] font-bold text-gray-800 transition-colors hover:bg-yellow-400 sm:mt-0 sm:w-auto"/>
            </div>
          </div>
          ...
          <div className="relative mt-16 sm:mt-12 lg:mt-0 lg:w-1/2">
            <div className={`relative ${circlePositionClass}`}>
              ...
              <div className={`relative rounded-full bg-yellow-300 ${circleSizeClass}`}>
                ...
                <div className={`absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 overflow-visible ${imageContainerClass}`}>
                  <Image src="/images/bakery/breadbasket.png" alt="Basket of fresh bread and bakery products" fill sizes="(max-width: 640px) 350px, (max-width: 1024px) 450px, 600px" priority className={`object-contain ${imageScaleClass}`}/>
                </div>
              </div>
            </div>
          </div>
