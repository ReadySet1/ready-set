 ...
 */
export function FormSection({ children, formName, preserveFormData = true, enableRetry = true, }: FormSectionProps) {
    ...
    return (<ComponentErrorBoundary componentName={`Form: ${formName}`} graceful={true} enableRetry={enableRetry} onError={handleError}>
      <div className="relative">
        {children}

        {/* Retry indicator for form sections */}
        {retryState.isRetrying && (<div className="absolute inset-0 z-10 flex items-center justify-center rounded-md bg-white/80 dark:bg-gray-900/80">
 ...
 */
export function FormField({ children, fieldName, graceful = true, }: FormFieldProps) {
    return (<ComponentErrorBoundary componentName={`FormField: ${fieldName}`} graceful={graceful} enableRetry={false} // Form fields typically don't retry individually
    >
      {children}
    </ComponentErrorBoundary>);
}
 ...
 */
export function FormFieldErrorFallback({ fieldName }: {
    fieldName: string;
}) {
    return (<div className="rounded-md border border-orange-200 bg-orange-50 p-3 dark:bg-orange-900/20">
