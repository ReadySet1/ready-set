...
import { Search, Truck, Phone, CheckCircle, RadioTower, X, Info, UserCheck, AlertCircle, Clock, MapPin, } from "lucide-react";
...
const DriverAssignmentDialog: React.FC<DriverAssignmentDialogProps> = ({ isOpen, onOpenChange, isDriverAssigned, drivers, selectedDriver, onDriverSelection, onAssignOrEditDriver, }) => {
    ...
    useEffect(() => {
        if (searchTerm.trim() === "") {
            ...
            const filtered = drivers.filter((driver) => (driver.name?.toLowerCase().includes(lowercaseSearch) ?? false) ||
    ...
    return (<Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="z-[9999] m-4 flex max-h-[90vh] w-[95vw] max-w-[700px] flex-col gap-0 overflow-y-auto rounded-2xl border-none bg-white p-0 shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 sm:px-6">
          ...
          <AnimatePresence>
            {selectedDriver && (<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="mb-4">
                ...
                <div className="flex items-center gap-3 rounded-xl border border-primary/30 bg-gradient-to-r from-yellow-50 to-primary/10 p-3 shadow-sm sm:p-4">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <Badge variant="outline" className="flex-shrink-0 border-primary/30 bg-primary/20 font-medium text-primary shadow-sm">
          ...
          <div className="relative mb-4">
            ...
            <Input placeholder="Search drivers by name or phone..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="h-11 rounded-lg border-slate-200 pl-9 pr-9 transition-colors focus:border-primary focus:ring-primary/20"/>
            {searchTerm && (<button onClick={clearSearch} className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400 transition-colors hover:text-slate-600">
                <X className="h-4 w-4"/>
              </button>)}
          </div>

          {currentDrivers.length === 0 ? (<motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.2 }} className="py-8 text-center sm:py-12">
            ...
            </motion.div>) : (<div className="overflow-hidden rounded-xl border border-slate-200 shadow-sm">
              {/* Desktop Table View */}
              <div className="hidden sm:block">
                <ScrollArea className="h-[280px]">
                  <Table>
                    ...
                    <TableBody>
                      <AnimatePresence>
                        {currentDrivers.map((driver, index) => (<motion.tr key={driver.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2, delay: index * 0.05 }} className={cn("border-b transition-colors hover:bg-slate-100/50 data-[state=selected]:bg-slate-100 dark:hover:bg-slate-800/50 dark:data-[state=selected]:bg-slate-800", selectedDriver === driver.id
                            ...
                            <TableCell className="py-4">
                              <div className="flex items-center gap-3">
                                <Avatar className={`h-10 w-10 border-2 ${selectedDriver === driver.id ? "border-primary/30 shadow-sm" : "border-slate-200"}`}>
                                  <AvatarFallback className={selectedDriver === driver.id
                    ...
                    : "bg-slate-100 text-slate-600"}>
                            ...
                            <TableCell>
                              <Badge variant="outline" className="border-green-200 bg-green-50 text-green-700 shadow-sm">
                                <RadioTower className="mr-1 h-3 w-3"/>
                                Active
                              </Badge>
                            </TableCell>
                            <TableCell className="text-right">
                              <Button onClick={() => handleDriverSelection(driver.id)} variant={selectedDriver === driver.id
                    ? "default"
                    : "outline"} size="sm" className={selectedDriver === driver.id
              ...
              <div className="sm:hidden">
                <ScrollArea className="h-[280px] px-2">
                  <div className="space-y-3 py-2">
                    <AnimatePresence>
                      {currentDrivers.map((driver, index) => (<motion.div key={driver.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2, delay: index * 0.05 }} className={`rounded-lg border p-3 transition-all ${selectedDriver === driver.id
                    ...
                    ? "bg-gradient-to-r from-primary to-custom-yellow font-medium text-white shadow-sm hover:from-primary/90 hover:to-custom-yellow/90"
                    : "border-slate-200 text-slate-600 transition-all hover:border-primary/30 hover:bg-yellow-50 hover:text-primary"}>
                              {selectedDriver === driver.id ? (<CheckCircle className="h-4 w-4"/>) : ("Select")}
                            </Button>
                          </div>
                        </motion.div>))}
                    </AnimatePresence>
              ...
              {totalPages > 1 && (<div className="border-t bg-gradient-to-r from-slate-50 to-white p-3">
                  <Pagination>
                    <PaginationContent className="flex-wrap gap-1">
                      <PaginationItem>
                        <PaginationPrevious onClick={() => handlePageChange(Math.max(1, currentPage - 1))} className={`cursor-pointer text-sm transition-colors hover:text-primary ${currentPage === 1 ? "pointer-events-none opacity-50" : ""}`}/>
                      </PaginationItem>
                    ...
                    return (<PaginationItem key={page}>
                              <PaginationLink onClick={() => handlePageChange(page)} isActive={currentPage === page} className={`cursor-pointer text-sm transition-all ${currentPage === page
                            ? "bg-gradient-to-r from-primary to-custom-yellow font-medium text-white shadow-sm hover:from-primary/90 hover:to-custom-yellow/90"
                      ...
                      <PaginationItem>
                        <PaginationNext onClick={() => handlePageChange(Math.min(totalPages, currentPage + 1))} className={`cursor-pointer text-sm transition-colors hover:text-primary ${currentPage === totalPages ? "pointer-events-none opacity-50" : ""}`}/>
                      </PaginationItem>
                    </PaginationContent>
                  </Pagination>
                </div>)}
        ...
        <DialogFooter className="mt-auto flex flex-col-reverse gap-3 border-t bg-gradient-to-r from-slate-50 to-white p-4 sm:flex-row sm:gap-2 sm:p-6">
          <Button variant="outline" onClick={() => onOpenChange(false)} className="h-11 w-full border-slate-200 text-slate-700 transition-colors hover:bg-slate-100 sm:w-auto">
            Cancel
          </Button>
          <Button onClick={handleFinalAssignment} disabled={!selectedDriver || isAssigning} className="h-11 w-full bg-gradient-to-r from-primary to-custom-yellow font-medium text-white shadow-sm transition-all hover:from-primary/90 hover:to-custom-yellow/90 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto">
            {isAssigning ? (<>
