...
import { postsQuery, postQuery, guideQuery, guidesQuery, allCategoriesQuery, categoryGuidesQuery } from "./queries";
...
export async function getGuideBySlug(slug: string): Promise<Guide> {
    try {
        const guide = await client.fetch(`*[_type == "guide" && slug.current == $slug][0]{
        _id,
        _type,
        _updatedAt,
        title,
