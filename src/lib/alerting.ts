...
class AlertStore {
    ...
    add(alert: Alert): void {
        ...
        const existingAlert = this.alerts.find(a => a.fingerprint === alert.fingerprint && !a.resolved);
        if (existingAlert) {
 ...
 */
export function createAlert(type: AlertType, severity: AlertSeverity, title: string, description: string, source: string, data: Record<string, any> = {}): Alert {
...
export function monitorErrorThresholds(thresholds: Partial<AlertThresholds> = {}): void {
    ...
    if (criticalErrors >= config.criticalErrorsPerHour) {
        createAlert(AlertType.ERROR_THRESHOLD, AlertSeverity.CRITICAL, 'Critical Error Threshold Exceeded', `${criticalErrors} critical errors detected in the last hour (threshold: ${config.criticalErrorsPerHour})`, 'error-monitoring', {
    ...
    if (highErrors >= config.highErrorsPerHour) {
        createAlert(AlertType.ERROR_THRESHOLD, AlertSeverity.WARNING, 'High Error Threshold Exceeded', `${highErrors} high-severity errors detected in the last hour (threshold: ${config.highErrorsPerHour})`, 'error-monitoring', {
    ...
    if (errorRate >= config.errorRatePercentage) {
        createAlert(AlertType.PERFORMANCE_DEGRADATION, AlertSeverity.WARNING, 'High Error Rate Detected', `Error rate is ${errorRate.toFixed(2)}% (threshold: ${config.errorRatePercentage}%)`, 'error-monitoring', {
            errorRate,
...
export function monitorAuthFailures(failures: number, timeWindowMinutes: number = 1): void {
    ...
    if (failures >= threshold) {
        createAlert(AlertType.SECURITY_INCIDENT, AlertSeverity.CRITICAL, 'Authentication Failure Spike', `${failures} authentication failures in ${timeWindowMinutes} minute(s) (threshold: ${threshold})`, 'auth-monitoring', {
...
export function monitorDatabaseHealth(connectionFailures: number): void {
    ...
    if (connectionFailures >= threshold) {
        createAlert(AlertType.SERVICE_DOWN, AlertSeverity.EMERGENCY, 'Database Connection Failures', `${connectionFailures} database connection failures detected (threshold: ${threshold})`, 'database-monitoring', {
...
export function monitorApiPerformance(endpoint: string, responseTime: number): void {
    ...
    if (responseTime >= threshold) {
        createAlert(AlertType.PERFORMANCE_DEGRADATION, responseTime >= threshold * 2 ? AlertSeverity.CRITICAL : AlertSeverity.WARNING, 'API Performance Degradation', `${endpoint} responded in ${responseTime}ms (threshold: ${threshold}ms)`, 'performance-monitoring', {
            endpoint,
            responseTime,
            threshold,
...
export function monitorMemoryUsage(): void {
    ...
    if (usagePercentage >= threshold) {
        createAlert(AlertType.RESOURCE_EXHAUSTION, usagePercentage >= 95 ? AlertSeverity.CRITICAL : AlertSeverity.WARNING, 'High Memory Usage', `Memory usage is ${usagePercentage.toFixed(2)}% (threshold: ${threshold}%)`, 'resource-monitoring', {
            usagePercentage,
            threshold,
            memoryUsage: {
...
export function monitorErrorPatterns(error: StructuredError): void {
    ...
    if (error.category === ErrorCategory.AUTH) {
        if (error.message.includes('brute force') ||
            ...
            createAlert(AlertType.SECURITY_INCIDENT, AlertSeverity.CRITICAL, 'Potential Brute Force Attack', `Authentication security incident detected: ${error.message}`, 'security-monitoring', {
                errorId: error.id,
    ...
    if (error.category === ErrorCategory.DATABASE && error.severity === ErrorSeverity.CRITICAL) {
        createAlert(AlertType.SERVICE_DOWN, AlertSeverity.CRITICAL, 'Database Service Issue', `Critical database error: ${error.message}`, 'database-monitoring', {
            errorId: error.id,
    ...
    if (error.category === ErrorCategory.PAYMENT) {
        createAlert(AlertType.ERROR_THRESHOLD, AlertSeverity.CRITICAL, 'Payment Processing Error', `Payment system error: ${error.message}`, 'payment-monitoring', {
            errorId: error.id,
            endpoint: error.context.request?.endpoint,
