...
import { DashboardMetrics, DashboardMetricsError, dashboardQuerySchema, DASHBOARD_CACHE_CONFIG, PERFORMANCE_THRESHOLDS } from '@/types/dashboard';
import { getDashboardMetricsCache, getDashboardMetricsCacheWithEtag, setDashboardMetricsCache, invalidateDashboardMetricsCache, generateDashboardMetricsCacheKey } from '@/lib/cache/dashboard-cache';
import { recordApiPerformance } from '@/lib/monitoring/dashboard-performance';
import { CACHE_CONFIGS, handleConditionalRequest, createCachedResponse, recordCacheMetrics } from '@/lib/cache/http-cache';
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        if (authError || !user) {
            return NextResponse.json({ error: 'Unauthorized' } as DashboardMetricsError, { status: 401 });
        }
        ...
        if (profileError || !profile?.type) {
            ...
            return NextResponse.json({ error: 'User profile not found' } as DashboardMetricsError, { status: 404 });
        }
        // Type guard for user type
        const userType = profile.type as string;
        if (!['ADMIN', 'SUPER_ADMIN', 'HELPDESK', 'VENDOR'].includes(userType)) {
            return NextResponse.json({ error: 'Insufficient permissions' } as DashboardMetricsError, { status: 403 });
        }
        ...
        try {
            ...
            if (validationError instanceof z.ZodError) {
                ...
                } as DashboardMetricsError, { status: 400 });
            }
        ...
        const conditionalResponse = handleConditionalRequest(request, cachedResult.data, CACHE_CONFIGS.DASHBOARD_METRICS);
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json({ error: 'Failed to invalidate cache' } as DashboardMetricsError, {
            status: 500,
            headers: {
                'X-Response-Time': `${duration.toFixed(2)}ms`,
            }
