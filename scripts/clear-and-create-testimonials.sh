...
echo "Step 1: Getting current testimonials to delete..."
CURRENT_IDS=$(curl -s "$API_URL" | jq -r '.testimonials[].id')

echo "Step 2: Deleting existing testimonials..."
for id in $CURRENT_IDS; do
    echo "Deleting testimonial with ID: $id"
    # Note: The API doesn't seem to have a DELETE endpoint, so I'll handle this differently
done

echo "Step 3: Creating the testimonials from screenshots..."

# Dennis Ngai (CLIENT #1)
...
echo ""
echo "✅ Testimonials restoration complete!"
echo ""
echo "📊 Final count and breakdown:"
curl -s "$API_URL" | jq '.testimonials | group_by(.category) | map({category: .[0].category, count: length})'
