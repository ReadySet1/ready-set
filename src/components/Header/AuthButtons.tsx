...
const ROLE_DISPLAY_INFO: Record<UserType, {
    title: string;
    path: string;
}> = {
    [UserType.VENDOR]: {
...
const AuthButtons: React.FC<AuthButtonsProps> = ({ sticky, pathUrl }) => {
    ...
    if (user && userRole) {
        ...
        <Link href={roleInfo.path}>
          <p className={`loginBtn px-7 py-3 text-base font-medium ${!sticky && pathUrl === "/" ? "text-white" : "text-black"}`}>
            {roleInfo.title}
          </p>
        </Link>
...
const SignOutButton: React.FC<ButtonProps> = ({ sticky, pathUrl }) => {
    ...
    return (<button onClick={handleSignOut} className={`signUpBtn rounded-lg px-6 py-3 text-base font-medium duration-300 ease-in-out ${pathUrl !== "/" || sticky
...
const SignInButton: React.FC<ButtonProps> = ({ sticky, pathUrl }) => (<Link href="/sign-in" className={`px-7 py-3 text-base font-medium hover:opacity-70 ${pathUrl !== "/" || sticky
        ? "dark:text-white"
        : "text-black dark:text-white"}`}>
    Sign In
  </Link>);
const SignUpButton: React.FC<ButtonProps> = ({ sticky, pathUrl }) => (<Link href="/sign-up" className={`rounded-lg px-6 py-3 text-base font-medium text-white duration-300 ease-in-out ${pathUrl !== "/" || sticky
