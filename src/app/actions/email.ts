...
const determineNotificationType = (data: FormInputs) => {
    if (data.subject?.includes("Vendor Registration"))
        return "vendor";
    if (data.subject?.includes("Client Registration"))
        return "client";
    if (data.subject?.includes("Food Delivery"))
        return "delivery";
...
export async function sendDeliveryNotifications(data: DeliveryNotificationData): Promise<{
    success: boolean;
    error?: string;
}> {
    try {
