...
export default function MileageReportClient() {
  ...
  const totalKm = rows.reduce((sum, row) => sum + (row.totalDistanceKm || 0), 0);
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-border/50 mt-4">
            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
              ...
              <span>
                <strong>Total Distance:</strong> {Math.round(totalKm * 10) / 10} km
              </span>
              <span>
              ...
              {rows.map((row) => (
                <tr key={row.shiftId} className="border-b last:border-0">
                  ...
                  <td className="px-2 py-2 text-right">
                    {Math.round(row.totalDistanceKm * 10) / 10}
                  </td>
                  <td className="px-2 py-2 text-right">
                    {row.deliveryCount}
                  </td>
