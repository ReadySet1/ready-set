...
const OrderSkeleton: React.FC = () => (<div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
...
const SingleOrder: React.FC<SingleOrderProps> = ({ onDeleteSuccess, showHeader = true, canAssignDriver = false, canUpdateDriverStatus = false, canDeleteOrder = false, canEditOrder = false, 
// Granular visibility props with default values
canViewOrderTitle = false, canViewOrderStatus = false, canViewOrderNumber = false, canViewDeliveryDate = false, canViewDeliveryTime = false, }) => {
    ...
    const fetchOrderDetails = useCallback(async () => {
        ...
        try {
            ...
            const orderResponse = await fetch(`/api/orders/${encodeURIComponent(orderNumber)}?include=dispatch.driver`, {
                credentials: "include",
                headers: {
            ...
            try {
                const filesResponse = await fetch(`/api/orders/${encodeURIComponent(orderNumber)}/files`, {
                    credentials: "include",
                    headers: {
                        Authorization: `Bearer ${session.access_token}`,
                        "Content-Type": "application/json",
    ...
    const handleAssignOrEditDriver = async () => {
        ...
        try {
            ...
            toast.success(isDriverAssigned
    ...
    const handleOrderStatusChange = async (newStatus: OrderStatus) => {
        ...
        try {
            ...
            if (!updatedOrder)
                return; // Handle case where promise returns undefined due to auth error
    ...
    const effectivePermissions = {
        canViewOrderTitle: canViewOrderTitle || userRoles.isAdmin || userRoles.isSuperAdmin,
        canViewOrderStatus: canViewOrderStatus || userRoles.isAdmin || userRoles.isSuperAdmin,
        canViewOrderNumber: canViewOrderNumber || userRoles.isAdmin || userRoles.isSuperAdmin,
        canViewDeliveryDate: canViewDeliveryDate || userRoles.isAdmin || userRoles.isSuperAdmin,
        canViewDeliveryTime: canViewDeliveryTime || userRoles.isAdmin || userRoles.isSuperAdmin,
    };
    ...
    if (!order) {
        return (<div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-50 via-white to-slate-50">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3 }} className="mx-4 max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-xl">
          ...
          <Button variant="default" onClick={() => window.history.back()} className="rounded-xl bg-blue-600 px-6 py-2 text-white shadow-lg transition-all duration-200 hover:bg-blue-700 hover:shadow-xl">
            Go Back
          </Button>
        </motion.div>
      </div>);
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className={`container mx-auto px-6 pb-8 ${effectivePermissions.canViewOrderTitle ? "pt-8" : "pt-16"}`}>
        {/* Modern Header */}
        <div className="mb-8">
          {/* Title Section */}
          <div className={`mb-6 ${effectivePermissions.canViewOrderTitle ? "mt-8" : "mt-20"}`}>
            <div className="flex items-start justify-between">
              <div className="flex-1">
                {/* Order Title and Status - Only visible to admin/super admin */}
                {effectivePermissions.canViewOrderTitle && (<div className="mb-4 flex items-center gap-3">
                    <h1 className="text-3xl font-bold text-slate-800">
                      {order.order_type === "catering"
                ? "Catering Request"
                : "On-Demand Order"}
                    </h1>
                    {effectivePermissions.canViewOrderStatus && (<Badge className="border-slate-200 bg-slate-100 px-3 py-1 text-sm font-medium text-slate-700">
            ...
            effectivePermissions.canViewDeliveryTime) && (<div className="flex items-center gap-6 text-sm text-slate-600">
                    {effectivePermissions.canViewOrderNumber && (<div className="flex items-center gap-2">
                        <Package className="h-4 w-4 text-slate-500"/>
                        <span className="font-medium">{order.orderNumber}</span>
                      </div>)}
                    {effectivePermissions.canViewDeliveryDate &&
                order.pickupDateTime && (<div className="flex items-center gap-2">
              ...
              {canUserAssignDriver && (<div className="ml-6">
                  <Button onClick={handleOpenDriverDialog} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-white shadow-sm hover:bg-blue-700">
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          ...
          <div className="space-y-8 lg:col-span-2">
        ...
        }} driverInfo={driverInfo} updateDriverStatus={updateDriverStatus} canAssignDriver={canUserAssignDriver} canUpdateDriverStatus={canUpdateDriverStatus} onAssignDriver={handleOpenDriverDialog} showAssignDriverButton={false}/>
                <Separator />
                <OrderStatusCard order_type={order.order_type} initialStatus={order.status} orderId={order.id} onStatusChange={handleOrderStatusChange} canChangeStatus={userRoles.isAdmin || userRoles.isSuperAdmin}/>
              </div>
            </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  {order.pickupAddress && (<div className="space-y-3">
                  ...
                  {order.deliveryAddress && (<div className="space-y-3">
                      <div className="flex items-center gap-2 text-sm font-medium text-slate-600">
            ...
            order.pickupNotes ||
            order.specialNotes) && (<div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="border-b border-slate-100 p-6">
                  <h2 className="flex items-center gap-2 text-xl font-semibold text-slate-800">
                ...
                <div className="p-6">
                  <AdditionalInfo clientAttention={order.clientAttention} pickupNotes={order.pickupNotes} specialNotes={order.specialNotes}/>
                </div>
              </div>)}
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="p-6">
                <OrderFilesManager orderNumber={order.orderNumber} order_type={order.order_type} orderId={order.id.toString()} initialFiles={files}/>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  {order.user?.email && (<div>
                      ...
                      <div className="flex items-center gap-2 text-slate-800">
                        ...
                        <a href={`mailto:${order.user.email}`} className="transition-colors hover:text-blue-600">
                          {order.user.email}
                        </a>
                      </div>
                    </div>)}
                  {order.user?.contactNumber && (<div>
                      <div className="mb-1 text-sm font-medium text-slate-500">
                        Phone
                      </div>
                      <div className="flex items-center gap-2 text-slate-800">
                        <Phone className="h-4 w-4 text-slate-400"/>
                        <a href={`tel:${order.user.contactNumber}`} className="transition-colors hover:text-blue-600">
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="space-y-3 p-6">
                <Button variant="outline" className="w-full justify-start gap-2 hover:bg-slate-50" onClick={() => window.print()}>
                  <FileText className="h-4 w-4"/>
                  Print Order
                </Button>
                {order.user?.email && (<Button variant="outline" className="w-full justify-start gap-2 hover:bg-slate-50" onClick={() => window.open(`mailto:${order.user?.email}?subject=Regarding Order ${order.orderNumber}`)}>
                    <Mail className="h-4 w-4"/>
                    Email Customer
                  </Button>)}
                {order.user?.contactNumber && (<Button variant="outline" className="w-full justify-start gap-2 hover:bg-slate-50" onClick={() => window.open(`tel:${order.user?.contactNumber}`)}>
            ...
            : "N/A"}
                  ...
                  {order.pickupDateTime && (<div className="flex items-start gap-3">
                  ...
                  {order.arrivalDateTime && (<div className="flex items-start gap-3">
                  ...
                  {order.completeDateTime && (<div className="flex items-start gap-3">
      ...
      <DriverAssignmentDialog isOpen={isDriverDialogOpen} onOpenChange={(open) => {
            setIsDriverDialogOpen(open);
        }} isDriverAssigned={isDriverAssigned} drivers={drivers} selectedDriver={selectedDriver} onDriverSelection={handleDriverSelection} onAssignOrEditDriver={handleAssignOrEditDriver}/>
    </div>);
};
