...
import { UploadErrorHandler, RetryHandler, FileValidator, DEFAULT_RETRY_CONFIG, DEFAULT_VALIDATION_CONFIG } from "@/lib/upload-error-handler";
import { UploadError, UploadErrorType, UploadProgress, UploadSession, UploadOptions } from "@/types/upload";
...
export function useUploadFile({ bucketName = "fileUploader", defaultUploadedFiles = [], maxFileCount = 5, maxFileSize = 10 * 1024 * 1024, // 10MB by default
allowedFileTypes = [], entityType, userId, entityId: initialEntityId, category, }: UseUploadFileOptions = {}) {
    ...
    const onUpload = useCallback(async (files: FileWithPath[]): Promise<UploadedFile[]> => {
        ...
        try {
            ...
            for (let i = 0; i < files.length; i++) {
                const file = files[i];
                if (!file)
                    continue; // Skip if file is somehow undefined
                const progressIndex = i;
                const fileProgress = fileProgresses[progressIndex];
                if (!fileProgress)
                    continue; // Skip if fileProgress is undefined
                ...
                let response;
                try {
                    response = await RetryHandler.withRetry(uploadOperation, {
                ...
                if (!result.success || !result.file) {
                    ...
                    fileProgress.status = 'error';
                    fileProgress.error = UploadErrorHandler.createValidationError('content', 'Invalid server response', 'Upload completed but server returned invalid response.');
                    session.failedFiles++;
                    delete newProgresses[fileKey];
                    setProgresses({ ...newProgresses });
                    setUploadSession({ ...session });
    ...
    const updateEntityId = useCallback(async (newEntityId: string) => {
        try {
    ...
    const deleteFileWithSupabase = useCallback(async (fileKey: string) => {
        try {
            // First get the file information from the API
            const infoResponse = await fetch(`/api/file-uploads/info?fileId=${fileKey}`, {
                method: "GET",
            });
            if (!infoResponse.ok) {
