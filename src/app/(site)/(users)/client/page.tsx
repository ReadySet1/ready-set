...
import { DashboardCardSkeleton, OrderCardSkeleton, QuickActionsSkeleton, } from "@/components/Skeleton/AuthSkeleton";
...
async function getClientDashboardData(userId: string): Promise<ClientDashboardData> {
    ...
    const [recentCateringOrders, recentOnDemandOrders] = await withDatabaseRetry(async () => {
        return Promise.all([
    ...
    const stats = await withDatabaseRetry(async () => {
        ...
        const [activeCateringCount, activeOnDemandCount, completedCateringCount, completedOnDemandCount, savedLocationsCount,] = await Promise.all([
    ...
    }, 3, "client dashboard stats");
    return {
        recentOrders: combinedOrders,
        stats: {
            activeOrders: (stats.activeCateringCount ?? 0) + (stats.activeOnDemandCount ?? 0),
            completedOrders: (stats.completedCateringCount ?? 0) +
...
const UpcomingOrderCard = ({ order }: {
    ...
    return (<div className="rounded-lg border border-gray-100 bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
      ...
      <div className="flex items-center justify-between border-t border-gray-100 pt-2">
        ...
        <Link href={orderDetailsLink} className="text-sm font-medium text-primary hover:underline">
          View Details
        </Link>
      </div>
    </div>);
...
const ClientDashboardSkeleton: React.FC = () => (<div className="grid grid-cols-1 gap-6 md:grid-cols-3">
...
const ClientDashboardContent = ({ data }: {
    ...
    return (<div className="grid grid-cols-1 gap-6 md:grid-cols-3">
      ...
      <div className="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm md:col-span-2">
        ...
        <div className="p-5">
          {hasRecentOrders ? (<div className="space-y-4">
              {data.recentOrders.map((order) => (<UpcomingOrderCard key={`${order.orderType}-${order.id}`} order={order}/>))}
            </div>) : (<div className="py-8 text-center">
              <p className="mb-4 text-gray-500">
                You haven't placed any orders yet
              </p>
              <Link href="/catering-request" className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-center font-medium text-white hover:bg-opacity-90">
      ...
      <div className="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
        ...
        <div className="p-5">
          <div className="space-y-4">
            <Link href="/catering-request" className="flex items-center rounded-lg border border-gray-100 p-3 transition-colors hover:bg-gray-50">
            ...
            <Link href="/addresses" className="flex items-center rounded-lg border border-gray-100 p-3 transition-colors hover:bg-gray-50">
            ...
            <Link href="/profile" className="flex items-center rounded-lg border border-gray-100 p-3 transition-colors hover:bg-gray-50">
            ...
            <Link href="/contact" className="flex items-center rounded-lg border border-gray-100 p-3 transition-colors hover:bg-gray-50">
...
const ClientPage = async () => {
    ...
    const isAllowedRole = userRole.toUpperCase() === UserType.CLIENT ||
